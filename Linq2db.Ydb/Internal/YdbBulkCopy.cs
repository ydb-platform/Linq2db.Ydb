...
namespace Linq2db.Ydb.Internal
{
    ...
    public class YdbBulkCopy : BasicBulkCopy
    {
        ...
        protected override BulkCopyRowsCopied ProviderSpecificCopy<T>(ITable<T> table, DataOptions options,
            IEnumerable<T> source)
        {
        ...
        protected override async Task<BulkCopyRowsCopied> ProviderSpecificCopyAsync<T>(ITable<T> table,
            DataOptions options, IAsyncEnumerable<T> source, CancellationToken cancellationToken)
        {
            if (table.TryGetDataConnection(out var dataConnection))
            {
        ...
        protected override async Task<BulkCopyRowsCopied> ProviderSpecificCopyAsync<T>(ITable<T> table,
            DataOptions options, IEnumerable<T> source, CancellationToken cancellationToken)
        {
            if (table.TryGetDataConnection(out var dataConnection))
            {
                var connection = _provider.TryGetProviderConnection(dataConnection,
        ...
        {
            var sqlBuilder =
                (YdbSqlBuilder)_provider.CreateSqlBuilder(table.DataContext.MappingSchema, dataConnection.Options);
            var ed = table.DataContext.MappingSchema.GetEntityDescriptor(typeof(T),
                dataConnection.Options.ConnectionOptions.OnEntityDescriptorCreated);
            var columns = ed.Columns
                .Where(c => !c.SkipOnInsert || options.BulkCopyOptions.KeepIdentity == true && c.IsIdentity).ToList();
            ...
            while (await rd.ReadAsync(cancellationToken).ConfigureAwait(false))
            {
                ...
                GetAsParameters(rd, dataConnection, command.CreateParameter, row);
            ...
            if (currentCount > 0 && options.BulkCopyOptions.NotifyAfter != 0 &&
                options.BulkCopyOptions.RowsCopiedCallback != null)
                options.BulkCopyOptions.RowsCopiedCallback(rowsCopied);
        ...
        // Todo remove and replace for rd.GetAsParameters(command.CreateParameter, row);
        private static int GetAsParameters<T>(
            BulkCopyReader<T> rd,
            DataConnection dataConnection,
            Func<DbParameter> parameterFactory,
            object?[] values)
            where T : notnull
        {
            return rd.GetValues(values);
        }
